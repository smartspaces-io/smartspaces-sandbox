...
public class StandardBluezBluetoothDevice implements BluetoothDevice {
   ...
   */
  public StandardBluezBluetoothDevice(StandardBluezBluetoothProvider bluezProvider,
  ...
  public void startup() throws Exception {
    ...
    bluezDevice = (Device1) dbusConnection.getRemoteObject(BluezConstants.BLUEZ_DBUS_BUSNAME,
        deviceObjectPath, Device1.class);

    System.out.println("Got bluez device " + bluezDevice);

    bluezDeviceProperties = (Properties) dbusConnection
        .getRemoteObject(BluezConstants.BLUEZ_DBUS_BUSNAME, deviceObjectPath, Properties.class);
    ...
    dbusConnection.addSigHandler(Properties.PropertiesChanged.class,
        bluezProvider.getBluezDbusBusName(), bluezDeviceProperties, propertiesChangedSignalHandler);
  }
   ...
   */
  private void
      handlePropertiesChangedDbusSignal(final Properties.PropertiesChanged propertiesChanged) {
    Runnable run = new Runnable() {
      @Override
      public void run() {
        System.out.println(
            "Properties changed for Bluetooth device " + propertiesChanged.getPropertiesChanged());
      }
    };
