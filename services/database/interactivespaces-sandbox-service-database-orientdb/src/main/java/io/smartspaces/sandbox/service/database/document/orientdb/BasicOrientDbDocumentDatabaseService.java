...
public class BasicOrientDbDocumentDatabaseService extends BaseSupportedService {
	 ...
	 */
	public static final String PROTOCOL_IMPLEMENTATION = "com.orientechnologies.orient.server.network.protocol.binary.ONetworkProtocolBinary";
	 ...
	 * will be kept in memory the first time they are loaded. Changes to the
	 * files will be taken on the next restart.
	 */
	public static final String CONFIGURATION_CACHE_STATIC_RESOURCES = "server.cache.staticResources";

	/**
	 * Configuration property for the logging level of the logger that outputs
	 * to the console.
	 */
	 ...
	 */
	public OrientDbDocumentDatabaseEndpoint getOrientDbDocumentDatabaseEndpoint(String databaseName, Log log) {
		String databaseUrl = DATABASE_URL_PROTOCOL_PLOCAL
				+ getSpaceEnvironment().getFilesystem().getDataDirectory(getName()).getAbsolutePath() + File.separator
				+ databaseName;
	 ...
	 * @return new endpoint for accessing the database
	 */
	public OrientDbDocumentDatabaseEndpoint getOrientDbDocumentDatabaseEndpoint(File dbDirectory, Log log) {
	 ...
	 * @return new endpoint for accessing the database
	 */
	public OrientDbDocumentDatabaseEndpoint getOrientDbDocumentDatabaseEndpoint(String databaseUrl, String login,
	 ...
	 * created documents are stored in this database. A connection that is
	 * created through an endpoint is automatically activated in a thread it is
	 * created in.
	 *
	 * @param database
	...
	private static class DatabaseServiceStarter {
		...
		public synchronized void startup() {
			if (orientDbServer == null) {
				...
				OServerNetworkProtocolConfiguration protocol = new OServerNetworkProtocolConfiguration(protocolName,
						implementation);
				...
				OServerUserConfiguration user = new OServerUserConfiguration(SERVER_USER_LOGIN, SERVER_USER_PASSWORD,
