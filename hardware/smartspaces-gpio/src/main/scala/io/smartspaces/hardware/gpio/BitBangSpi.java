...
public class BitBangSpi implements Spi {
  ...
  /**
   * {@code true} if the clock base is high, {@code false} if the clock base is
   * low.
   */
  private boolean clockBase;

  /**
   * {@code true} if a bit read should be on the leading edge of a clock shift.
   */
  ...
  public void startup() {
    ...
    sclkOutput = gpio.provisionDigitalOutputPin(sclkPin, "SCLK Pin",
        (clockBase) ? PinState.HIGH : PinState.LOW);
