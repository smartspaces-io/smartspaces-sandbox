...
public class StandardGpio {
	...
	public static void main(String[] args) throws Exception {
		...
		// create a pin listener to print out changes to the output gpio pin
		// state
		output.addListener(new GpioPinListenerDigital() {
			...
			public void handleGpioPinDigitalStateChangeEvent(GpioPinDigitalStateChangeEvent event) {
				...
				console.println(" --> GPIO PIN STATE CHANGE: " + event.getPin() + " = "
						+ ConsoleColor.conditional(event.getState().isHigh(), // conditional
																				// expression
								ConsoleColor.GREEN, // positive conditional
													// color
		...
		console.println("--> [" + output.toString() + "] state was provisioned with state = "
				+ ConsoleColor.conditional(output.getState().isHigh(), // conditional
																		// expression
		...
		output.pulse(1000, true); // set second argument to 'true' use a
									// blocking call
		Thread.sleep(50);
